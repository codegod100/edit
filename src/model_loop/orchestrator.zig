...
/// Tool usage output with background highlight (full width)
pub fn toolUsage(comptime fmt: []const u8, args: anytype) void {
    if (g_tool_output_callback) |callback| {
        var buf: [4096]u8 = undefined;
        const text = std.fmt.bufPrint(&buf, fmt, args) catch return;
        
        // Get terminal width and pad to full width
        const term_width = display.terminalColumns();
        const text_len = display.visibleLenAnsi(text);
        
        // Calculate padding needed
        if (term_width > text_len) {
            var full_buf: [8192]u8 = undefined;
            const padding_len = term_width - text_len;
            
            // All text in cyan on background
            const full_text = std.fmt.bufPrint(&full_buf, "{s}{s}{s}{s}{s}\n", .{
                display.C_REASONING_BG,
                display.C_CYAN,
                text,
                (" " ** 200)[0..@min(padding_len, 200)],
                display.C_RESET,
            }) catch return;
            
            if (g_tool_output_arena) |*arena| {
                const copy = arena.allocator().alloc(u8, full_text.len) catch return;
                @memcpy(copy, full_text);
                callback(copy);
            }
        } else {
            var colored_buf: [4096]u8 = undefined;
            const colored_text = std.fmt.bufPrint(&colored_buf, "{s}{s}{s}{s}\n", .{
                display.C_REASONING_BG,
                display.C_CYAN,
                text,
                display.C_RESET,
            ...
            if (g_tool_output_arena) |*arena| {
                const copy = arena.allocator().alloc(u8, colored_text.len) catch return;
                @memcpy(copy, colored_text);
                callback(copy);
            }
        }
    }
...
pub fn runModel(
    ...
    const system_prompt = custom_system_prompt orelse "You are a highly capable software engineering assistant. Your goal is to help the user with their task efficiently and accurately.\n\n" ++
        ...
        "4. **Tools & Output**: Use `bash` with `rg` for searching. Read files with `offset` and `limit`. Never scan the entire filesystem (`find /`, recursive `/` searches). Stay in the project/task workspace. Never use `sudo` in tool calls. You will receive tool outputs with ANSI colors removed for clarity. Always verify your changes if possible.\n" ++
    ...
    if (std.mem.startsWith(u8, user_input, "[") and std.mem.endsWith(u8, user_input, "]")) {
        ...
        try w.appendSlice(arena_alloc, "{\"role\":\"system\",\"content\":");
        try w.writer(arena_alloc).print("{f}", .{std.json.fmt(system_prompt, .{})});
        try w.appendSlice(arena_alloc, "},");

        try w.appendSlice(arena_alloc, "{\"role\":\"user\",\"content\":");
        try w.writer(arena_alloc).print("{f}", .{std.json.fmt(user_input, .{})});
        try w.appendSlice(arena_alloc, "}");
    }
    ...
    while (iter < max_iterations) : (iter += 1) {
        ...
        if (response.reasoning.len > 0) {
            ...
            if (trimmed_reasoning.len > 0) {
                try logger.transcriptWrite("\n[Reasoning]\n{s}\n", .{trimmed_reasoning});
                display.addTimelineEntry("\n", .{});
                display.addWrappedTimelineEntry(display.C_DIM, trimmed_reasoning, display.C_RESET);
            }
        }
        ...
        for (response.tool_calls) |tc| {
                ...
                if (std.mem.eql(u8, tc.tool, "read") or std.mem.eql(u8, tc.tool, "read_file")) {
                    ...
                    if (try tools.parseReadParamsFromArgs(arena_alloc, tc.args)) |params| {
                        if (params.offset) |off| {
                            toolUsage("reading {s} [{d}:{d}]", .{ path, off, params.limit orelse 0 });
                        } else {
                            toolUsage("reading {s}", .{path});
                        }
                    } else {
                        toolUsage("reading {s}", .{path});
                    }
                } else if (std.mem.eql(u8, tc.tool, "write_file")) {
                    display.setSpinnerState(.writing);
                    toolUsage("writing {s}", .{path});
                } else if (std.mem.eql(u8, tc.tool, "edit_file")) {
                    display.setSpinnerState(.writing);
                    toolUsage("editing {s}", .{path});
                } else if (std.mem.eql(u8, tc.tool, "list_files") or std.mem.eql(u8, tc.tool, "list")) {
                    display.setSpinnerState(.tool);
                    toolUsage("listing files in {s}", .{path});
                } else {
                    display.setSpinnerState(.writing);
                    toolUsage("{s} {s}", .{ tc.tool, path });
                }
            ...
            } else if (std.mem.eql(u8, tc.tool, "web_search") or std.mem.eql(u8, tc.tool, "web_fetch")) {
                display.setSpinnerState(.search);
                toolUsage("{s}", .{tc.tool});
            } else if (std.mem.eql(u8, tc.tool, "subagent_spawn")) {
                display.setSpinnerState(.tool);
                toolUsage("spawning subagent", .{});
            } else {
                display.setSpinnerState(.tool);
                toolUsage("{s}", .{tc.tool});
            }
